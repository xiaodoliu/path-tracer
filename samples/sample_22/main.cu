...
int main(){
    ...
    host_materials[0] = material_data{material_type::LAMBERTIAN, color(0.5, 0.5, 0.5)}; // ground
        ...
        for(int b = -11; b < 11; ++b){
            ...
            if((center-point3(4, 0.2, 0)).length() > 0.9){
                if(choose_mat < 0.8){
                    ...
                    host_materials[host_material_index++] = material_data{material_type::METAL, albedo, fuzz};
                }else{
                    host_materials[host_material_index++] = material_data{material_type::DIELECTRIC, color(1.0, 1.0, 1.0), /*fuzz=*/0.0, /*refraction_index=*/1.5};
                }
    ...
    host_materials[host_material_index++] = material_data{material_type::DIELECTRIC, color(.0, 1.0, 1.0), /*fuzz=*/0.0, /*refraction_index=*/1.5};
    ...
    host_materials[host_material_index++] = material_data{material_type::METAL, color(0.7, 0.6, 0.5), /*fuzz=*/0.0};
    host_objects[host_object_index++] = scene_object{object_type::SPHERE, sphere(point3(4, 1, 0), 1.0, host_material_index - 1)};
    ...
    int actual_num_materials = host_material_index;
    
    cam.render(image_width, image_height, host_objects, actual_num_objects, host_materials, actual_num_materials);
