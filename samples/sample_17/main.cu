...
int main(){
    ...
    host_materials[0] = material_data{material_type::LAMBERTIAN, color(0., 0., 1.)}; // left
    host_materials[1] = material_data{material_type::LAMBERTIAN, color(1., 0., 0.)}; // right
    ...
    host_objects[0].sphere_data = sphere(point3(-R, 0, -1), R, /*material_id=*/0);
    host_objects[1].type = object_type::SPHERE;
    host_objects[1].sphere_data = sphere(point3(R, 0, -1), R, /*material_id=*/1);

    // camera
    double focal_length = 1.0;
    double vfov = 90;
    camera cam;
    cam.init(image_width, /*samples_per_pixel=*/100, /*max_depth=*/50, aspect_ratio, focal_length, vfov, point3(0, 0, 0));
