...
D inline double hittable_pdf_value(const point3& origin, const vec3& direction, const hittable& objects){
    return objects.pdf_value(origin, direction);
}

D inline vec3 hittable_pdf_generate(const point3& origin, const hittable& objects, curandState* state){
