...
D color ray_color(const ray& r, int depth, 
    ...
    while(0 < depth--){
        ...
        if(!scatter(cur, rec, scattered_attenuation, scattered, 
            device_textures, mat, pdf_val, state)){
            return radiance;
        }
        if(light_index != -1){
            assert(device_objects[light_index].type == object_type::QUAD);
            quad light = device_objects[light_index].quad_data;
            scattered = ray(rec.p, hittable_pdf_generate(rec.p, light, state), cur.time());
            pdf_val = hittable_pdf_value(rec.p, scattered.direction(), light);
        }else{
            scattered = ray(rec.p, cosine_pdf_generate(rec.normal, state), cur.time());
            pdf_val = cosine_pdf_value(scattered.direction(), rec.normal);
        }
        double spdf = scattering_pdf(cur, rec, scattered, mat);
        throughput *= scattered_attenuation * spdf / pdf_val;
...
void cornell_box(){
    ...
    camera cam;
    cam.init(/*image_width=*/600, /*samples_per_pixel=*/10, /*max_depth=*/50, 
