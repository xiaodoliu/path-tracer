...
D color ray_color(const ray& r, int depth, 
    ...
    while(0 < depth--){
        ...
        color color_from_emission = emitted(rec.u, rec.v, rec.p, device_textures, mat);
        ...
        switch(mat.type){
            ...
            case material_type::ISOTROPIC: {
                auto on_light = point3(random_double(213, 343, state), 554, random_double(227, 332, state));
                ...
                auto light_cosine = std::fabs(to_light.y()); // not general but works for cornell box where the light plane is parallel to the y-axis
                if(light_cosine < 0.000001){
                    return radiance;
                }
                pdf_val = distance_squared / (light_area * light_cosine);
                scattered = ray(rec.p, to_light, cur.time());
                double spdf = scattering_pdf(cur, rec, scattered, mat);
                throughput *= scattered_attenuation * spdf / pdf_val;
                break;
            }
            default:
                throughput *= scattered_attenuation;
        }
        cur = scattered;
    }
    return color(0, 0, 0);
