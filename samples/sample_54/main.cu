...
D color ray_color(const ray& r, int depth, 
    ...
    while(0 < depth--){
        ...
        double pdf_val = 0.0;
        if(light_index != -1){
            assert(device_objects[light_index].type == object_type::QUAD);
            scattered = ray(rec.p, 
                mixture_pdf_generate(rec.p, rec.normal, device_objects[light_index], srec.pdf_type, state), 
                    cur.time());
                pdf_val = mixture_pdf_value(rec.p, scattered.direction(), rec.normal, device_objects[light_index], srec.pdf_type);
        }else{
            scattered = ray(rec.p, material_pdf_generate(rec.normal, srec.pdf_type, state), cur.time());
            pdf_val = material_pdf_value(scattered.direction(), rec.normal, srec.pdf_type);
        }
...
__global__ void render_kernel(unsigned char* image, int width, int height, 
    ...
    bvh_node* device_bvh_nodes, int num_bvh_nodes, int root_node_index,
    int* device_prim_indices, int num_prim_indices, int light_index = -1,
    ...
    for(int s_i = 0; s_i < cam.sqrt_spp; ++s_i){
        for(int s_j = 0; s_j < cam.sqrt_spp; ++s_j){
            ...
            pixel_color += ray_color(r, cam.max_depth, cam.background,
                ...
                device_prim_indices, num_prim_indices, light_index, &local_rand_state);
        }
    }  
...
void camera::render(int width, int height, 
    ...
    int* prim_indices, int num_prim_indices, int light_index){
    ...
    render_kernel<<<grid_size, block_size>>>(image, width, height,
        *this, device_objects, num_objects, 
        ...
        device_materials, num_materials, 
        device_bvh_nodes, num_bvh_nodes, root_node_index,
        device_prim_indices, num_prim_indices, light_index,
        rand_states);
    CUDA_CHECK(cudaGetLastError());
...
void cornell_box(){
    ...
    host_materials[4] = material_data{material_type::METAL, color(0.8, 0.85, 0.88), /*texture_id=*/-1, /*fuzz=*/0.0}; // metal
    ...
    host_objects[6].box_data = box(point3(0, 0, 0), point3(165, 330, 165), /*material_id=*/4, /*angle=*/15, /*offset=*/vec3(265, 0, 295));
    ...
    cam.render(cam.image_width, cam.image_height, 
        ...
        host_materials, num_materials,
        host_bvh_nodes.data(), host_bvh_nodes.size(), root_node_index, 
        prim_indices.data(), actual_num_objects, light_index);
}
