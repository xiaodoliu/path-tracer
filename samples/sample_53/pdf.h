...
D inline double mixture_pdf_value(const point3& origin, const vec3& direction, const vec3& normal, const scene_object& light){
    double p0 = light_pdf_value(origin, direction, light);
    double p1 = cosine_pdf_value(direction, normal);
    return 0.5 * p0 + 0.5 * p1;
}

D inline vec3 mixture_pdf_generate(const point3& origin, const vec3& normal, const scene_object& light, curandState* state) {
    if(random_double(state) < 0.5) {
        return light_pdf_generate(origin, light, state);
    }
    return cosine_pdf_generate(normal, state);
}