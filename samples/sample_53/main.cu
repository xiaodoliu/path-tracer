...
D color ray_color(const ray& r, int depth, 
    ...
    while(0 < depth--){
        ...
        if(light_index != -1){
            ...
            scattered = ray(rec.p, mixture_pdf_generate(rec.p, rec.normal, device_objects[light_index], state), cur.time());
            pdf_val = mixture_pdf_value(rec.p, scattered.direction(), rec.normal, device_objects[light_index]);
        }else{
