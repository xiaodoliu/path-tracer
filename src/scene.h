...
inline void add_quad_transformed(scene_object* objects, int& object_index, const point3& Q,
                                 const vec3& u, const vec3& v, int meterial_id, double angle,
                                 const vec3& offset) {
    objects[object_index].type = object_type::QUAD;
    objects[object_index++].quad_data =
        quad(rotate_y(Q, angle) + offset, rotate_y(u, angle), rotate_y(v, angle), meterial_id);
}
...
D bool hit_bvh(const ray& r, interval ray_t, hit_record& rec, const bvh_node* nodes,
               const int* prim_indices, const scene_object* objects, curandState* state = nullptr) {
    int stack[64];
    int sp = 0;
    stack[sp++] = 0;
...
int build_bvh(std::vector<bvh_node>& nodes, std::vector<int>& prim_indices,
              const scene_object* objects, int start, int end) {
