...
struct camera{
    ...
    void init(int image_width, int samples_per_pixel, int max_depth,
        ...
        point3 lookfrom, point3 lookat, vec3 vup, double defocus_angle = 0.0, double focus_dist = 10.0, color background = color(0.0, 0.0, 0.0)){
    ...
    D ray get_ray(int i, int j, int s_i, int s_j, curandState* state){
        ...
        auto pixel_sample = pixel_sample_start + 
            (i + offset.x()) * viewport_pixel_u_delta +
    ...
    void render(int width, int height,
        ...
        int* prim_indices, int num_prim_indices, int* light_indices = nullptr, int num_lights = 0, int frame = 0);
