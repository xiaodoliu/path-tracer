...
HD inline double degrees_to_radians(double degrees) { return degrees * pi / 180.0; }
...
H inline int random_int(int min, int max) { return static_cast<int>(random_double(min, max + 1)); }

D inline double random_double(curandState* state) {
    return 1.0 - curand_uniform(state);  // rand val in [0, 1)
}
